...
const systemChangelogs = [
    ...
    },
    {
        ...
        changes: [
            ...
            "A-EMT kann parallel mit Doctor- und Paramedic-Rang geführt und angezeigt werden.",
            "Sanktionen schließen nach dem Speichern der Bearbeitung und erhalten formatierte Zeilenumbrüche.",
            "Allgemeine Notizen können bearbeitet und gelöscht werden.",
            "Aktive Abwesenheiten können im Mitarbeiterkalender bearbeitet oder frühzeitig beendet werden.",
            "Personalabteilung kann zusätzlich CLS, EHK und Luftrettung vergeben."
        ]
    },
    {
    ...
    },
    {
        ...
        changes: [
            "Ausgestellte Sanktionen können von berechtigten Personalmitarbeitern direkt in der Personalhistorie bearbeitet werden.",
            "Sanktionen können nach Sicherheitsabfrage gelöscht werden.",
            "Bearbeiten und Löschen werden im Admin-Audit protokolliert."
        ]
    },
    {
        id: "sys_v6_8_11", version: "v6.8.11", date: "22.09.2026", ts: 1790046000000,
        category: "Fehlerbehebung", title: "Personalakten-Dropdowns und Ränge korrigiert",
        changes: [
            "Vereidigung, Erste-Hilfe-Kurs, Behandlungseinweisung und interner Personalstatus sind für berechtigte Personalmitarbeiter direkt auswählbar.",
            "Laufbahn und Ränge wurden übersichtlicher strukturiert und zeigen den aktuellen Stand prominent an.",
            "Die aktuellen Ränge der SAMD-Masterliste werden einmalig verbindlich nach Dienstnummer übernommen.",
            "Spätere manuelle Rang- und Laufbahnänderungen werden als manuell markiert und nicht durch den Masterlisten-Abgleich zurückgesetzt."
        ]
    },
    {
        id: "sys_v6_8_10", version: "v6.8.10", date: "22.09.2026", ts: 1790042400000,
        category: "Verbesserung", title: "Personalakten mit Masterliste abgeglichen",
        changes: [
            "Stammdaten, Qualifikationen, interne Funktionen, Laufbahnen und aktuelle Ränge können aus der SAMD-Masterliste anhand der Dienstnummer übernommen werden.",
            "Diensttage berücksichtigen die in der Masterliste hinterlegte Diensttage-Korrektur.",
            "Die Stammdatenkarte besitzt jetzt einen klaren Bearbeiten-Modus mit Stift-Symbol.",
            "Eingabe- und Auswahlfelder der Personalverwaltung wurden an das dunkle MMD-Cloud-Design angepasst.",
            "E-Mail-Adressen, Discord-IDs und alte Foto-Verweise werden weiterhin nicht übernommen."
        ]
    },
    {
        id: "sys_v6_8_9", version: "v6.8.9", date: "22.09.2026", ts: 1790038800000,
        category: "Neue Funktion", title: "Personalakten und Kündigungsarchiv erweitert",
        changes: [
            "Personalakten enthalten jetzt zusätzliche Stammdaten und Qualifikationen wie Telefonnummer, Zweitjob, interne Funktionen, Einweisung, Einstellung, Vereidigung, Erste-Hilfe-Kurs und Behandlungseinweisung.",
            "Interne Personalstatus wie Perso-Ticket offen, Inaktivität prüfen und Kündigung prüfen erscheinen ausschließlich im Personalbereich.",
            "Die Personalübersicht zeigt offene Personalthemen und zählt sie direkt am Personalreiter.",
            "Kündigungen und Austritte werden in einem eigenen Archiv mit Datum, letztem Rang, Grund, Bearbeiter und Diensttagen dokumentiert.",
            "Wiedereinstellungen reaktivieren das bestehende Konto und führen dieselbe Personalakte und Historie weiter.",
            "Einstellungen, Abwesenheiten, Rückkehr, Rangänderungen, Kündigungen und Wiedereinstellungen werden als Personalereignisse chronologisch protokolliert.",
            "E-Mail-Adressen und Discord-IDs werden nicht gespeichert oder übernommen."
        ]
    },
    {
...
const defaultRoles = {
    ...
    personalabteilung: {
        ...
        canEditPrices:false, canEditGuide:false, canEditCommands:false, canEditLinks:false,
        canViewChiefMaterials:false, canEditChiefMaterials:false,
        canManageMemberAccess:true, canManageCareerPaths:true, canViewPersonnelRecords:true, canManagePersonnelRecords:true, canManagePersonnelAbsences:true, canManagePersonnelRanks:true, canManagePersonnelDepartures:true, canCreateEmployees:true, canManageMaintenance:false,
        canEditSanctionsCatalog:true,
        canViewExamSolutions:false,
...
const ROLE_PROPERTY_MAP = {
    ...
    roleFlagManagePersonnelDepartures: 'canManagePersonnelDepartures',
...
function getServiceDaysForUser(user) {
    const seed=getPersonnelMasterlistSeedForUser(user),raw=String(user?.einstellungsDatum || seed?.employmentDate || '').trim();
    if (!raw) return null;
    const start = new Date(raw + 'T00:00:00');
    if (Number.isNaN(start.getTime())) return null;
    const today = new Date(); today.setHours(0,0,0,0);
    const correctionRaw=user?.diensttageKorrektur;
    const correction=(correctionRaw===undefined||correctionRaw===null||correctionRaw==='')?Number(seed?.serviceDaysCorrection||0):Number(correctionRaw||0);
    return Math.max(0, Math.floor((today - start) / 86400000) + 1 + (Number.isFinite(correction)?correction:0));
}
...
function berechneDienstTage(shouldPersist = false) {
    ...
    const seed=getPersonnelMasterlistSeedForUser(sessionUser),correctionRaw=sessionUser?.diensttageKorrektur;
    const correction=(correctionRaw===undefined||correctionRaw===null||correctionRaw==='')?Number(seed?.serviceDaysCorrection||0):Number(correctionRaw||0);
    const ed = new Date(f.value); ed.setHours(0,0,0,0);
    const h = new Date(); h.setHours(0,0,0,0);
    const t = Math.max(0, Math.floor((h - ed)/(1000*60*60*24)) + 1 + (Number.isFinite(correction)?correction:0));
...
const PERSONNEL_MEMBER_ROLE_ASSIGNMENT_IDS = new Set(['ausbildungsleitung', 'ausbilder', 'mitarbeiter', 'cls', 'ehk', 'luftrettung']);
...
function getAssignableRoleIdsForOperator(user = sessionUser) {
    ...
    if (operatorRoleIds.includes('personalabteilung')) {
        // Personalabteilung: bestehende Mitgliedsrollen plus CLS, EHK und Luftrettung.
        return new Set([...PERSONNEL_MEMBER_ROLE_ASSIGNMENT_IDS].filter(roleId => !!cachedRoles[roleId]));
    }

    // Ausbildungsleitung und andere Rollen mit Mitgliedsrecht behalten den bisherigen eingeschränkten Umfang.
...
function getPersonnelPermissions(){
    ...
    return {canAccess:!!(master||eff.canManageCareerPaths||eff.canViewPersonnelRecords||eff.canManagePersonnelRecords||eff.canManagePersonnelAbsences||eff.canManagePersonnelRanks||eff.canManagePersonnelDepartures||eff.canCreateEmployees),
        canViewRecords:!!(master||eff.canViewPersonnelRecords||eff.canManagePersonnelRecords||eff.canManagePersonnelRanks||eff.canManagePersonnelDepartures),canManageRecords:!!(master||eff.canManagePersonnelRecords),
        canManageAbsences:!!(master||eff.canManagePersonnelAbsences),canManageRanks:!!(master||eff.canManagePersonnelRanks),canManageCareer:!!(master||eff.canManageCareerPaths),canManageDepartures:!!(master||(eff.canManagePersonnelDepartures&&eff.canManageMemberAccess)),canCreateEmployees:!!(master||eff.canCreateEmployees)};
}
...
function getEmployeeRankData(uId){const x=cachedEmployeeRanks?.[uId]||{},stored={common:normalizePersonnelRankKey(x.common),doctor:normalizePersonnelRankKey(x.doctor),paramedic:normalizePersonnelRankKey(x.paramedic)};if(stored.common||stored.doctor||stored.paramedic)return stored;const seed=getPersonnelMasterlistSeedForUser(cachedUsers?.[uId]);return seed?.rank?buildMasterlistRankState(seed.rank):stored;}
function getEmployeeRankKeys(uId){const r=getEmployeeRankData(uId),career=getEmployeeCareerPathKey(uId);if(r.common&&PERSONNEL_RANKS[r.common]?.track==='common-high')return[r.common];const b=[];if(r.common)b.push(r.common);if((career==='doctor'||career==='both'||career==='none')&&r.doctor)b.push(r.doctor);if((career==='paramedic'||career==='both'||career==='none')&&r.paramedic)b.push(r.paramedic);return b;}
...
const PERSONNEL_INTERNAL_STATUSES=Object.freeze({
    none:{label:'Kein offener Status',icon:'✅'}, ticket:{label:'Perso-Ticket offen',icon:'🟡'}, inactive:{label:'Inaktivität prüfen',icon:'🟠'}, termination_review:{label:'Kündigung prüfen',icon:'🔴'}
});
const PERSONNEL_MASTERLIST_SEED=Object.freeze([{"dn":"1","name":"Marcus Ryan","employmentDate":"2025-05-01","serviceDaysCorrection":405,"rank":"chief","career":"both","profile":{"phone":"205 24045510","secondJob":"","additionalFunctions":"Leitung, Luftretterausbilder, EHK Ausbilder","instructedBy":"","hiredBy":"","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"2","name":"Andy Harper","employmentDate":"2025-03-31","serviceDaysCorrection":67,"rank":"ass_chief","career":"both","profile":{"phone":"907 11110815","secondJob":"Bean Machine","additionalFunctions":"Stellv. Leitung, Luftretterausbilder, EHK Ausbilder","instructedBy":"","hiredBy":"","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"4","name":"Nick Garcia","employmentDate":"2026-01-10","serviceDaysCorrection":161,"rank":"domo","career":"both","profile":{"phone":"480 85825243","secondJob":"Club 77 / Inhaber","additionalFunctions":"Personalabteilungsleitung, Luftrettung, EHK Ausbilder","instructedBy":"Marcus Ryan","hiredBy":"Marcus Ryan","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"5","name":"Mike Gonzalo","employmentDate":"2026-01-13","serviceDaysCorrection":16,"rank":"fod","career":"both","profile":{"phone":"205 40000000","secondJob":"","additionalFunctions":"Ausbildungsabteilungsleitung, Luftretterausbilder","instructedBy":"Marcus Ryan","hiredBy":"Marcus Ryan","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"6","name":"Katarina Harper","employmentDate":"2025-11-03","serviceDaysCorrection":120,"rank":"chief_physician","career":"both","profile":{"phone":"480 04254628","secondJob":"","additionalFunctions":"Psychologieleitung, Luftrettung, EHK Ausbilder","instructedBy":"Andy Harper","hiredBy":"Andy Harper","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"7","name":"Tim Sanddorn","employmentDate":"2026-04-26","serviceDaysCorrection":0,"rank":"chief_physician","career":"both","profile":{"phone":"205 94708379","secondJob":"","additionalFunctions":"Stv. Leitung Psychologie, Luftretterausbilder","instructedBy":"Mike Gonzalo","hiredBy":"Nick Garcia","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"10","name":"Hutch Galiger","employmentDate":"2023-03-26","serviceDaysCorrection":0,"rank":"attending","career":"both","profile":{"phone":"205 42141080","secondJob":"IAA","additionalFunctions":"Luftretterausbilder, EHK Ausbilder","instructedBy":"Marcus Ryan","hiredBy":"Marcus Ryan","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"11","name":"Hiroto Takahashi","employmentDate":"2026-01-20","serviceDaysCorrection":0,"rank":"attending","career":"doctor","profile":{"phone":"502 16161616","secondJob":"","additionalFunctions":"","instructedBy":"Mike Gonzalo","hiredBy":"Nick Garcia","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"12","name":"Rene Stoned","employmentDate":"2026-04-30","serviceDaysCorrection":0,"rank":"medical_supervisor","career":"paramedic","profile":{"phone":"205 56640824","secondJob":"","additionalFunctions":"Stv. Leitung Ausbildungsabteilung, Luftretterausbilder, Luftrettung","instructedBy":"Mike Gonzalo","hiredBy":"Nick Garcia","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"13","name":"Mark Akuma","employmentDate":"2026-07-11","serviceDaysCorrection":0,"rank":"medical_supervisor","career":"paramedic","profile":{"phone":"205 97068710","secondJob":"Club77 / Geschäftsführung","additionalFunctions":"Personalabteilung, Luftrettung","instructedBy":"Mike Gonzalo","hiredBy":"Nick Garcia","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"14","name":"Sam Franzika","employmentDate":"2026-01-21","serviceDaysCorrection":0,"rank":"medical_supervisor","career":"paramedic","profile":{"phone":"480 37009278","secondJob":"","additionalFunctions":"Luftrettung","instructedBy":"Nick Garcia","hiredBy":"Nick Garcia","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"15","name":"Rico Malz","employmentDate":"2026-05-16","serviceDaysCorrection":0,"rank":"medical_supervisor","career":"none","profile":{"phone":"602 72624659","secondJob":"","additionalFunctions":"","instructedBy":"Nick Garcia","hiredBy":"Nick Garcia","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"27","name":"Ray Harper","employmentDate":"2026-05-25","serviceDaysCorrection":0,"rank":"physician","career":"none","profile":{"phone":"907 73894659","secondJob":"","additionalFunctions":"","instructedBy":"","hiredBy":"Nick Garcia","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"30","name":"Maximilian Miami","employmentDate":"2026-05-14","serviceDaysCorrection":0,"rank":"paramedic","career":"none","profile":{"phone":"602 40554286","secondJob":"","additionalFunctions":"","instructedBy":"Nick Garcia","hiredBy":"Nick Garcia","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"40","name":"Fabio Leroux","employmentDate":"2026-08-25","serviceDaysCorrection":0,"rank":"resident_physician","career":"none","profile":{"phone":"205 00000003","secondJob":"","additionalFunctions":"Luftrettung","instructedBy":"Rene Stoned","hiredBy":"Nick Garcia","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"41","name":"Conny Grey","employmentDate":"2026-03-27","serviceDaysCorrection":0,"rank":"a_emt","career":"none","profile":{"phone":"205 66896201","secondJob":"","additionalFunctions":"Luftrettung","instructedBy":"Nick Garcia","hiredBy":"Nick Garcia","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"no"}},{"dn":"42","name":"Domek Redfield","employmentDate":"2026-08-24","serviceDaysCorrection":0,"rank":"a_emt","career":"none","profile":{"phone":"205 77776969","secondJob":"","additionalFunctions":"","instructedBy":"Mark Akuma","hiredBy":"Tim Sanddorn","swornIn":"no","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"43","name":"Chiko Muerto","employmentDate":"2026-09-11","serviceDaysCorrection":0,"rank":"a_emt","career":"paramedic","profile":{"phone":"520 60841624","secondJob":"","additionalFunctions":"","instructedBy":"Nick Garcia","hiredBy":"Nick Garcia","swornIn":"no","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"44","name":"Brian Akuma","employmentDate":"2026-09-04","serviceDaysCorrection":0,"rank":"solo","career":"none","profile":{"phone":"480 88396899","secondJob":"","additionalFunctions":"","instructedBy":"Rico Malz","hiredBy":"Mark Akuma","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"45","name":"Alesya Leroux","employmentDate":"2026-08-25","serviceDaysCorrection":0,"rank":"solo","career":"none","profile":{"phone":"520 15958185","secondJob":"","additionalFunctions":"","instructedBy":"Rene Stoned","hiredBy":"Nick Garcia","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"46","name":"Luna Hunter","employmentDate":"2026-08-30","serviceDaysCorrection":0,"rank":"solo","career":"none","profile":{"phone":"602 51429565","secondJob":"","additionalFunctions":"","instructedBy":"Nick Garcia","hiredBy":"Nick Garcia","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"47","name":"John Fernandez Smith","employmentDate":"2026-09-08","serviceDaysCorrection":0,"rank":"solo","career":"none","profile":{"phone":"907 58793956","secondJob":"","additionalFunctions":"","instructedBy":"Nick Garcia","hiredBy":"Nick Garcia","swornIn":"no","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"48","name":"Nilo Leroux","employmentDate":"2026-09-06","serviceDaysCorrection":0,"rank":"solo","career":"none","profile":{"phone":"520 99666023","secondJob":"","additionalFunctions":"","instructedBy":"Rene Stoned","hiredBy":"Nick Garcia","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"49","name":"Neo Castilla","employmentDate":"2026-09-07","serviceDaysCorrection":0,"rank":"solo","career":"none","profile":{"phone":"205 77766699","secondJob":"","additionalFunctions":"","instructedBy":"Nick Garcia","hiredBy":"Nick Garcia","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"50","name":"Raven Marchetti","employmentDate":"2026-09-07","serviceDaysCorrection":0,"rank":"solo","career":"doctor","profile":{"phone":"907 43414151","secondJob":"Club77 / Bar","additionalFunctions":"","instructedBy":"Nick Garcia","hiredBy":"Nick Garcia","swornIn":"yes","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"51","name":"Rico Reimer","employmentDate":"2026-09-11","serviceDaysCorrection":0,"rank":"solo","career":"paramedic","profile":{"phone":"907 39498694","secondJob":"","additionalFunctions":"","instructedBy":"Nick Garcia","hiredBy":"Nick Garcia","swornIn":"no","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"52","name":"Andy Laken","employmentDate":"2026-09-11","serviceDaysCorrection":0,"rank":"trainee","career":"paramedic","profile":{"phone":"205 19892001","secondJob":"","additionalFunctions":"","instructedBy":"Nick Garcia","hiredBy":"Nick Garcia","swornIn":"no","firstAidCourse":"yes","treatmentInstruction":"yes"}},{"dn":"53","name":"Lars Petersen","employmentDate":"2026-09-12","serviceDaysCorrection":0,"rank":"solo","career":"paramedic","profile":{"phone":"480 35947610","secondJob":"","additionalFunctions":"","instructedBy":"Nick Garcia","hiredBy":"Nick Garcia","swornIn":"no","firstAidCourse":"yes","treatmentInstruction":"yes"}}]);
function normalizePersonnelDn(v){return String(v??'').replace(/\D/g,'').replace(/^0+/,'')||'0';}
function getPersonnelMasterlistSeedForUser(user){if(!user)return null;const dn=normalizePersonnelDn(user.dn);return PERSONNEL_MASTERLIST_SEED.find(x=>normalizePersonnelDn(x.dn)===dn)||null;}
    ...
    try{
        ...
        for(const seed of PERSONNEL_MASTERLIST_SEED){
            ...
            if(p.canManageRecords){
                ...
                if(!current){
                    set(`data/personnelProfiles/${uId}`,{...seed.profile,internalStatus:'none',updatedAt:now,updatedBy:author,updatedById:authorId});
                }else{
                    let touched=false;const fillText=(key,val)=>{if(val&&!String(current[key]||'').trim()){updates[`data/personnelProfiles/${uId}/${key}`]=val;touched=true;changed++;}};
                    fillText('phone',seed.profile.phone);fillText('secondJob',seed.profile.secondJob);fillText('additionalFunctions',seed.profile.additionalFunctions);fillText('instructedBy',seed.profile.instructedBy);fillText('hiredBy',seed.profile.hiredBy);
                    for(const key of ['swornIn','firstAidCourse','treatmentInstruction'])if(seed.profile[key]&&seed.profile[key]!=='unknown'&&(!current[key]||current[key]==='unknown')){updates[`data/personnelProfiles/${uId}/${key}`]=seed.profile[key];touched=true;changed++;}
                    if(touched){updates[`data/personnelProfiles/${uId}/updatedAt`]=now;updates[`data/personnelProfiles/${uId}/updatedBy`]=author;updates[`data/personnelProfiles/${uId}/updatedById`]=authorId;}
                }
            }
            if(p.canManageCareer&&seed.career&&seed.career!=='none'){
                const curCareer=careers[uId]||{};
                if(curCareer.source!=='manual'&&curCareer.source!==PERSONNEL_MASTERLIST_RANK_SOURCE)set(`data/employeeCareerPaths/${uId}`,{path:seed.career,source:PERSONNEL_MASTERLIST_RANK_SOURCE,updatedAt:now,updatedBy:author,updatedById:authorId});
            }
            if(p.canManageRanks&&seed.rank){
                const cur=ranks[uId]||{};
...
function getPersonnelProfile(uId){return cachedPersonnelProfiles?.[uId]||{};}
function getPersonnelInternalStatus(uId){const key=String(getPersonnelProfile(uId).internalStatus||'none');return PERSONNEL_INTERNAL_STATUSES[key]?key:'none';}
function personnelQualificationOptions(selected){
    const value=String(selected||'').toLowerCase()==='yes'?'yes':'no';
    return [['no','Nein'],['yes','Ja']].map(([k,label])=>`<option value="${k}" ${value===k?'selected':''}>${label}</option>`).join('');
}
function getPersonnelTrainerEntries(){
    return getPersonnelApprovedEntries().filter(([,user])=>{
        const roles=getUserRolesList(user);
        return roles.includes('ausbilder')||roles.includes('personalabteilung');
    });
}
function personnelTrainerOptions(selected=''){
    const current=String(selected||'').trim(),entries=getPersonnelTrainerEntries(),known=new Set(entries.map(([uId])=>personnelUserName(uId)));
    let out=`<option value="" ${current?'':'selected'}>Nicht angegeben</option>`;
    if(current&&!known.has(current))out+=`<option value="${escapeHtml(current)}" selected>${escapeHtml(current)} · Bestand</option>`;
    out+=entries.map(([uId,user])=>{const name=personnelUserName(uId),roles=getUserRolesList(user),label=[roles.includes('ausbilder')?'Ausbilder':'',roles.includes('personalabteilung')?'Perso':''].filter(Boolean).join(' + ');return `<option value="${escapeHtml(name)}" ${current===name?'selected':''}>${escapeHtml(formatStaffDn(user.dn))} · ${escapeHtml(name)} · ${escapeHtml(label)}</option>`;}).join('');
    return out;
}
...
function renderPersonnelWorkspace(){
    ...
    ['files','calendar','create','archive'].forEach(k=>{const b=document.getElementById('personnelNav_'+k);if(b)b.style.display=vis[k]?'':'none';});
    ...
    renderPersonnelOverview();renderPersonnelEmployeeList();renderPersonnelEmployeeDetail();renderPersonnelCalendar();renderPersonnelCreateForm();renderPersonnelArchive();
}
function renderPersonnelOverview(){
    const due=[],statusOpen=[];let vacation=0,excused=0,tickets=0,inactive=0,terminationReview=0;
    Object.entries(cachedPersonnelAbsences||{}).forEach(([uId,map])=>Object.entries(map||{}).forEach(([id,x])=>{if(!x||x.status==='returned'||x.active===false)return;if(x.type==='vacation')vacation++;if(x.type==='excused')excused++;if(isPersonnelAbsenceDue(x))due.push({uId,id,x});}));
    Object.entries(cachedPersonnelProfiles||{}).forEach(([uId,p])=>{const st=String(p?.internalStatus||'none');if(st==='ticket')tickets++;if(st==='inactive')inactive++;if(st==='termination_review')terminationReview++;if(st!=='none'&&PERSONNEL_INTERNAL_STATUSES[st])statusOpen.push({uId,status:st});});
    const set=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=String(v);};set('personnelStatEmployees',getPersonnelApprovedEntries().length);set('personnelStatVacation',vacation);set('personnelStatExcused',excused);set('personnelStatDue',due.length);set('personnelStatTickets',tickets);set('personnelStatInactive',inactive);set('personnelStatTermination',terminationReview);
    const navBadge=document.getElementById('personnelDueBadge'),openCount=due.length+tickets+inactive+terminationReview;if(navBadge){navBadge.textContent=String(openCount);navBadge.style.display=openCount?'inline-flex':'none';}
    const box=document.getElementById('personnelReturnAlerts');if(box){const canConfirm=getPersonnelPermissions().canManageAbsences;box.innerHTML=due.length?due.sort((a,b)=>String(a.x.endDate).localeCompare(String(b.x.endDate))).map(({uId,id,x})=>`<div class="personnel-return-alert"><div><b>${escapeHtml(personnelUserName(uId))}</b><span>${escapeHtml(PERSONNEL_ABSENCE_TYPES[x.type]?.label||'Abwesenheit')} endete am ${escapeHtml(formatPersonnelDate(x.endDate))}</span></div>${canConfirm?`<button type="button" class="btn personnel-small-btn" onclick="confirmPersonnelReturn('${uId}','${id}')">✅ Rückkehr bestätigen</button>`:'<strong>🔴 Rückkehr prüfen</strong>'}</div>`).join(''):'<div class="personnel-empty-state">✅ Keine offenen Rückkehrprüfungen.</div>';}
    const statusBox=document.getElementById('personnelStatusAlerts');if(statusBox)statusBox.innerHTML=statusOpen.length?statusOpen.map(({uId,status})=>`<button type="button" class="personnel-topic-row status-${status}" onclick="activePersonnelEmployeeId='${uId}';switchPersonnelView('files')"><span>${PERSONNEL_INTERNAL_STATUSES[status].icon}</span><div><b>${escapeHtml(personnelUserName(uId))}</b><small>${escapeHtml(PERSONNEL_INTERNAL_STATUSES[status].label)}</small></div><strong>Personalakte öffnen →</strong></button>`).join(''):'<div class="personnel-empty-state">✅ Keine offenen internen Personalthemen.</div>';
}
function renderPersonnelEmployeeList(){
    ...
    const entries=getPersonnelApprovedEntries().filter(([uId,u])=>!q||`${u.dn||''} ${u.vorname||''} ${u.nachname||''} ${getEmployeeRankLabels(uId).join(' ')} ${PERSONNEL_INTERNAL_STATUSES[getPersonnelInternalStatus(uId)]?.label||''}`.toLowerCase().includes(q));
    if(!activePersonnelEmployeeId||!cachedUsers[activePersonnelEmployeeId])activePersonnelEmployeeId=entries[0]?.[0]||'';
    box.innerHTML=entries.length?entries.map(([uId,u])=>{const internal=getPersonnelInternalStatus(uId);return `<button type="button" class="personnel-employee-row ${activePersonnelEmployeeId===uId?'active':''}" onclick="selectPersonnelEmployee('${uId}')"><span class="personnel-employee-dn">${escapeHtml(formatStaffDn(u.dn))}</span><span><b>${escapeHtml(personnelUserName(uId))}</b><small>${escapeHtml(getEmployeeRankLabels(uId).join(' · '))}</small></span><em class="personnel-row-signals">${getEmployeePublicAbsence(uId)?'●':''}${internal!=='none'?PERSONNEL_INTERNAL_STATUSES[internal].icon:''}</em></button>`;}).join(''):'<div class="personnel-empty-state">Keine Mitarbeiter gefunden.</div>';
}
function selectPersonnelEmployee(uId){activePersonnelEmployeeId=uId;personnelProfileEditingId='';activePersonnelSanctionEditId='';activePersonnelNoteEditId='';renderPersonnelEmployeeList();renderPersonnelEmployeeDetail();}
function togglePersonnelProfileEdit(uId,enabled){if(!getPersonnelPermissions().canManageRecords)return;personnelProfileEditingId=enabled?uId:'';renderPersonnelEmployeeDetail();if(enabled)setTimeout(()=>document.getElementById('personnelProfilePhone')?.focus(),0);}
function renderPersonnelTimelineItem(item,uId){
    const d=item.ts?new Date(item.ts).toLocaleString('de-DE'):'—',x=item.x||{},canManage=getPersonnelPermissions().canManageRecords;
...
function renderPersonnelEmployeeDetail(){
    ...
    Object.entries(record.sanctions||{}).forEach(([id,x])=>timeline.push({id,type:'sanction',ts:Number(x.createdAt)||0,x}));Object.entries(record.notes||{}).forEach(([id,x])=>timeline.push({id,type:'note',ts:Number(x.createdAt)||0,x}));Object.entries(record.rankups||{}).forEach(([id,x])=>timeline.push({id,type:'rankup',ts:Number(x.createdAt)||0,x}));Object.entries(record.events||{}).forEach(([id,x])=>timeline.push({id,type:'event',ts:Number(x.createdAt)||0,x}));timeline.sort((a,b)=>b.ts-a.ts);const abs=getPersonnelActiveAbsence(uId);
    box.innerHTML=`<div class="personnel-file-head"><span class="personnel-file-dn">${escapeHtml(formatStaffDn(user.dn))}</span><h3>${escapeHtml(personnelUserName(uId))}</h3><div class="personnel-file-badges">${getEmployeeCareerPathBadge(uId)} ${getEmployeeRankBadges(uId)} ${getEmployeeAbsenceBadge(uId)} ${internal!=='none'?`<span class="personnel-internal-badge status-${internal}">${PERSONNEL_INTERNAL_STATUSES[internal].icon} ${escapeHtml(PERSONNEL_INTERNAL_STATUSES[internal].label)}</span>`:''}</div></div>
    <div class="personnel-file-grid">
      ${p.canViewRecords?`<section class="personnel-card personnel-profile-card ${profileEditing?'is-editing':''}"><div class="personnel-card-title-row"><h4>🪪 Stammdaten & Qualifikationen</h4>${p.canManageRecords?`<button type="button" class="personnel-edit-icon ${profileEditing?'active':''}" onclick="togglePersonnelProfileEdit(\'${uId}\',${profileEditing?'false':'true'})" title="${profileEditing?'Bearbeiten beenden':'Stammdaten bearbeiten'}" aria-label="${profileEditing?'Bearbeiten beenden':'Stammdaten bearbeiten'}">${profileEditing?'✓':'✏️'}</button>`:''}</div><div class="personnel-readonly-row"><span>Eingestellt am</span><b>${escapeHtml(formatPersonnelDate(user.einstellungsDatum||getPersonnelMasterlistSeedForUser(user)?.employmentDate))}</b></div><div class="personnel-readonly-row"><span>Diensttage</span><b>${getServiceDaysForUser(user)??'—'}</b></div>
      <label>Telefonnummer</label><input id="personnelProfilePhone" value="${escapeHtml(profile.phone||'')}" ${p.canManageRecords?'':'disabled'} placeholder="Optional"><label>Zweitjob</label><input id="personnelProfileSecondJob" value="${escapeHtml(profile.secondJob||'')}" ${p.canManageRecords?'':'disabled'} placeholder="Optional"><label>Interne Funktionen / Zusatzaufgaben</label><textarea id="personnelProfileFunctions" rows="3" ${p.canManageRecords?'':'disabled'} placeholder="z. B. Personalabteilung, Luftrettung …">${escapeHtml(profile.additionalFunctions||'')}</textarea>
      <div class="personnel-two-col"><div><label>Eingewiesen durch</label><select id="personnelProfileInstructedBy" ${p.canManageRecords?'':'disabled'}>${personnelTrainerOptions(profile.instructedBy||'')}</select></div><div><label>Eingestellt durch</label><select id="personnelProfileHiredBy" ${p.canManageRecords?'':'disabled'}>${personnelTrainerOptions(profile.hiredBy||'')}</select></div></div>
      <div class="personnel-three-col personnel-qualification-grid"><div><label>Vereidigung</label><select id="personnelProfileSworn" ${p.canManageRecords?'':'disabled'}>${personnelQualificationOptions(profile.swornIn||'unknown')}</select></div><div><label>Erste-Hilfe-Kurs</label><select id="personnelProfileFirstAid" ${p.canManageRecords?'':'disabled'}>${personnelQualificationOptions(profile.firstAidCourse||'unknown')}</select></div><div><label>Behandlungseinweisung</label><select id="personnelProfileTreatment" ${p.canManageRecords?'':'disabled'}>${personnelQualificationOptions(profile.treatmentInstruction||'unknown')}</select></div></div>
      <label>Interner Personalstatus</label><select id="personnelProfileInternalStatus" ${p.canManageRecords?'':'disabled'}>${Object.entries(PERSONNEL_INTERNAL_STATUSES).map(([k,v])=>`<option value="${k}" ${internal===k?'selected':''}>${v.icon} ${escapeHtml(v.label)}</option>`).join('')}</select>
      ${p.canManageRecords?`<div class="personnel-edit-actions"><button type="button" class="btn personnel-primary-btn" onclick="savePersonnelProfile(\'${uId}\')">💾 Änderungen speichern</button></div>`:''}</section>`:'' }
      ${(p.canManageCareer||p.canManageRanks)?`<section class="personnel-card personnel-rank-card"><div class="personnel-card-title-row"><h4>🎖️ Laufbahn & Ränge</h4><span class="personnel-rank-source">${cachedEmployeeRanks?.[uId]?.source===PERSONNEL_MASTERLIST_RANK_SOURCE?'Masterliste':'Aktueller Stand'}</span></div>
      <div class="personnel-current-rank-summary"><div><span>Laufbahn</span><strong>${escapeHtml(EMPLOYEE_CAREER_PATHS[career]?.label||'Noch nicht festgelegt')}</strong></div><div><span>Aktueller Rang</span><strong>${escapeHtml(getEmployeeRankLabels(uId).join(' · '))}</strong></div></div>
      <p class="personnel-rank-help">Doctor- und Paramedic-Ränge können parallel geführt werden. Gemeinsame Führungsränge stehen darüber.</p>
      ${p.canManageCareer?`<div class="personnel-rank-section"><div class="personnel-rank-section-head"><b>🩺 Laufbahn</b><small>Doctor, Paramedic oder beide Laufbahnen</small></div><div class="personnel-inline-row"><select id="personnelCareerSelect"><option value="none" ${career==='none'?'selected':''}>Noch nicht festgelegt</option><option value="doctor" ${career==='doctor'?'selected':''}>Doctor</option><option value="paramedic" ${career==='paramedic'?'selected':''}>Paramedic</option><option value="both" ${career==='both'?'selected':''}>Doctor & Paramedic</option></select><button type="button" class="btn personnel-small-btn" onclick="savePersonnelCareer(\'${uId}\')">Laufbahn speichern</button></div></div>`:''}
      ${p.canManageRanks?`<div class="personnel-rank-section"><div class="personnel-rank-section-head"><b>🎖️ Rang festlegen</b><small>Aktuellen Rang auswählen; bei Änderungen ist ein Grund erforderlich.</small></div><div class="personnel-rank-fields"><div><label>Gemeinsamer Rang <small>Einstieg / Führung</small></label><select id="personnelRankCommon">${buildPersonnelRankOptions(PERSONNEL_COMMON_RANKS,ranks.common)}</select></div>${showDoctorRank?`<div><label>Doctor-Rang</label><select id="personnelRankDoctor">${buildPersonnelRankOptions(PERSONNEL_DOCTOR_RANKS,ranks.doctor)}</select></div>`:''}${showParamedicRank?`<div><label>Paramedic-Rang</label><select id="personnelRankParamedic">${buildPersonnelRankOptions(PERSONNEL_PARAMEDIC_RANKS,ranks.paramedic)}</select></div>`:''}</div><div class="personnel-rank-change-meta"><div><label>Grund der Rangänderung</label><select id="personnelRankReason"><option value="">Bitte wählen</option><option>Ausbildung</option><option>Wiedereinstellung</option><option>Sonder-Rankup</option><option>Korrektur</option></select></div><div><label>Zusatz / Begründung</label><input type="text" id="personnelRankNote" maxlength="180" placeholder="Optionaler Zusatz"></div></div><button type="button" class="btn personnel-primary-btn" onclick="savePersonnelRanks(\'${uId}\')">🎖️ Ränge speichern</button></div>`:''}
    </section>`:''}
      ${p.canManageRecords?`<section class="personnel-card"><h4>⚖️ Sanktion zur Akte</h4><input type="text" id="personnelSanctionTitle" maxlength="120" placeholder="Verstoß / Sanktion"><textarea id="personnelSanctionReport" rows="5" maxlength="3000" placeholder="Bericht zur Sanktion …"></textarea><button type="button" class="btn personnel-danger-btn" onclick="addPersonnelSanction('${uId}')">⚖️ Sanktion eintragen</button></section><section class="personnel-card"><h4>🗒️ Allgemeine Notiz</h4><textarea id="personnelNoteText" rows="5" maxlength="2000" placeholder="z. B. Namensänderung oder interner Hinweis …"></textarea><button type="button" class="btn personnel-primary-btn" onclick="addPersonnelNote('${uId}')">🗒️ Notiz hinzufügen</button></section>`:''}
...
async function terminatePersonnelEmployee(uId){
    const p=getPersonnelPermissions();if(!p.canManageDepartures)return;const user=cachedUsers?.[uId];if(!user)return;if(!requireTargetUserManagement(uId))return;const date=document.getElementById('personnelTerminationDate')?.value||personnelTodayIso(),reason=(document.getElementById('personnelTerminationReason')?.value||'').trim();if(!reason){alert('Bitte einen Kündigungsgrund eintragen.');return;}if(!confirm(`${personnelUserName(uId)} wirklich ins Kündigungsarchiv verschieben? Der Account bleibt bestehen, wird aber gesperrt.`))return;
    const now=Date.now(),author=personnelUserName(getUserAccountId(sessionUser)),authorId=getUserAccountId(sessionUser),ref=db.ref('data/personnelDepartures').push(),abs=getPersonnelActiveAbsence(uId),updates={};updates[`data/personnelDepartures/${ref.key}`]={userId:uId,dn:String(user.dn||''),name:personnelUserName(uId),employmentDate:String(user.einstellungsDatum||''),departureDate:date,serviceDays:getServiceDaysForUser(user)||0,terminatedBy:author,terminatedById:authorId,lastRank:getEmployeeRankLabels(uId).join(' · '),career:getEmployeeCareerPathLabel(uId),reason,createdAt:now};updates[`data/users/${uId}/status`]='revoked';updates[`data/employeeAbsenceStatus/${uId}`]=null;updates[`data/personnelProfiles/${uId}/internalStatus`]='none';updates[`data/personnelProfiles/${uId}/updatedAt`]=now;updates[`data/personnelProfiles/${uId}/updatedBy`]=author;updates[`data/personnelProfiles/${uId}/updatedById`]=authorId;
    if(abs){const found=Object.entries(cachedPersonnelAbsences?.[uId]||{}).find(([,x])=>x===abs);if(found){const aid=found[0];updates[`data/personnelAbsences/${uId}/${aid}/status`]='returned';updates[`data/personnelAbsences/${uId}/${aid}/active`]=false;updates[`data/personnelAbsences/${uId}/${aid}/returnedAt`]=now;updates[`data/personnelAbsences/${uId}/${aid}/returnedBy`]=author;updates[`data/personnelAbsences/${uId}/${aid}/returnedById`]=authorId;updates[`data/personnelAbsences/${uId}/${aid}/closedReason`]='termination';}}
...
async function rehirePersonnelEmployee(departureId){
    ...
    const now=Date.now(),today=personnelTodayIso(),author=personnelUserName(getUserAccountId(sessionUser)),authorId=getUserAccountId(sessionUser),updates={};updates[`data/users/${uId}/status`]='approved';updates[`data/personnelDepartures/${departureId}/rehiredAt`]=now;updates[`data/personnelDepartures/${departureId}/rehiredDate`]=today;updates[`data/personnelDepartures/${departureId}/rehiredBy`]=author;updates[`data/personnelDepartures/${departureId}/rehiredById`]=authorId;updates[`data/personnelProfiles/${uId}/internalStatus`]='none';updates[`data/personnelProfiles/${uId}/updatedAt`]=now;updates[`data/personnelProfiles/${uId}/updatedBy`]=author;updates[`data/personnelProfiles/${uId}/updatedById`]=authorId;
...
async function savePersonnelSanctionEdit(uId,sanctionId){
    if(!getPersonnelPermissions().canManageRecords)return;const current=cachedPersonnelRecords?.[uId]?.sanctions?.[sanctionId];if(!current)return;
    const title=(document.getElementById(`personnelSanctionEditTitle_${sanctionId}`)?.value||'').trim(),report=(document.getElementById(`personnelSanctionEditReport_${sanctionId}`)?.value||'').trim();
    if(!title||!report){alert('Titel und Bericht dürfen nicht leer sein.');return;}
    try{await db.ref(`data/personnelRecords/${uId}/sanctions/${sanctionId}`).update({title,report});activePersonnelSanctionEditId='';renderPersonnelEmployeeDetail();logAdminAudit('Sanktion bearbeitet',`${personnelUserName(uId)}: ${current.title||'Sanktion'} → ${title}`);showToast('✅ Sanktion wurde aktualisiert.','success');}
    catch(err){console.error(err);showToast('⚠️ Sanktion konnte nicht aktualisiert werden.','error',5000);}
}
async function deletePersonnelSanction(uId,sanctionId){
    if(!getPersonnelPermissions().canManageRecords)return;const current=cachedPersonnelRecords?.[uId]?.sanctions?.[sanctionId];if(!current)return;
    if(!confirm(`Sanktion „${current.title||'Sanktion'}“ bei ${personnelUserName(uId)} wirklich löschen? Dieser Vorgang kann nicht rückgängig gemacht werden.`))return;
...
async function savePersonnelNoteEdit(uId,noteId){
    if(!getPersonnelPermissions().canManageRecords)return;const current=cachedPersonnelRecords?.[uId]?.notes?.[noteId];if(!current)return;
    const text=(document.getElementById(`personnelNoteEditText_${noteId}`)?.value||'').trim();if(!text){alert('Die Notiz darf nicht leer sein.');return;}
    try{await db.ref(`data/personnelRecords/${uId}/notes/${noteId}`).update({text});activePersonnelNoteEditId='';renderPersonnelEmployeeDetail();logAdminAudit('Personalnotiz bearbeitet',`${personnelUserName(uId)}: Notiz aktualisiert`);showToast('✅ Notiz wurde aktualisiert.','success');}
    catch(err){console.error(err);showToast('⚠️ Notiz konnte nicht aktualisiert werden.','error',5000);}
}
async function deletePersonnelNote(uId,noteId){
    if(!getPersonnelPermissions().canManageRecords)return;const current=cachedPersonnelRecords?.[uId]?.notes?.[noteId];if(!current)return;
    if(!confirm(`Notiz bei ${personnelUserName(uId)} wirklich löschen? Dieser Vorgang kann nicht rückgängig gemacht werden.`))return;
    try{await db.ref(`data/personnelRecords/${uId}/notes/${noteId}`).remove();if(activePersonnelNoteEditId===`${uId}:${noteId}`)activePersonnelNoteEditId='';renderPersonnelEmployeeDetail();logAdminAudit('Personalnotiz gelöscht',`${personnelUserName(uId)}: Notiz gelöscht`);showToast('✅ Notiz wurde gelöscht.','success');}
    catch(err){console.error(err);showToast('⚠️ Notiz konnte nicht gelöscht werden.','error',5000);}
}
...
function renderPersonnelCalendar(){
    ...
    const ef=filterEmployee?.value||'all',rf=filterRank?.value||'all',rows=[];Object.entries(cachedPersonnelAbsences||{}).forEach(([uId,map])=>Object.entries(map||{}).forEach(([id,x])=>{if(!x||(ef!=='all'&&uId!==ef)||(rf!=='all'&&!getEmployeeRankKeys(uId).includes(rf)))return;rows.push({uId,id,x});}));rows.sort((a,b)=>String(b.x.startDate||'').localeCompare(String(a.x.startDate||'')));const canManage=getPersonnelPermissions().canManageAbsences;
    list.innerHTML=rows.length?rows.map(({uId,id,x})=>{
        const due=isPersonnelAbsenceDue(x),returned=x.status==='returned'||x.active===false,editKey=`${uId}:${id}`,editing=canManage&&!returned&&activePersonnelAbsenceEditId===editKey;
        if(editing)return `<div class="personnel-calendar-row editing"><div class="personnel-calendar-edit"><div class="personnel-calendar-edit-title"><b>✏️ Abwesenheit bearbeiten · ${escapeHtml(personnelUserName(uId))}</b><span>${escapeHtml(getEmployeeRankLabels(uId).join(' · '))}</span></div><div><label>Art</label><select id="personnelAbsenceEditType_${id}"><option value="vacation" ${x.type==='vacation'?'selected':''}>Urlaub</option><option value="excused" ${x.type==='excused'?'selected':''}>Entschuldigt abwesend</option></select></div><div><label>Von</label><input type="date" id="personnelAbsenceEditStart_${id}" value="${escapeHtml(x.startDate||'')}"></div><div><label>Bis</label><input type="date" id="personnelAbsenceEditEnd_${id}" value="${escapeHtml(x.endDate||'')}"></div><div class="personnel-calendar-edit-note"><label>Interne Notiz</label><input type="text" maxlength="240" id="personnelAbsenceEditNote_${id}" value="${escapeHtml(x.note||'')}"></div><div class="personnel-calendar-edit-actions"><button type="button" class="btn personnel-primary-btn" onclick="savePersonnelAbsenceEdit('${uId}','${id}')">💾 Speichern</button><button type="button" class="btn personnel-secondary-btn" onclick="cancelPersonnelAbsenceEdit()">Abbrechen</button></div></div></div>`;
        const actions=returned?'<span class="personnel-status-ok">✅ Rückkehr bestätigt</span>':canManage?`<div class="personnel-calendar-actions">${due?`<button type="button" class="btn personnel-small-btn danger" onclick="confirmPersonnelReturn('${uId}','${id}')">🔴 Rückkehr prüfen</button>`:`<span class="personnel-status-active">● Läuft</span><button type="button" class="btn personnel-small-btn danger" onclick="endPersonnelAbsenceEarly('${uId}','${id}')">⏹️ Frühzeitig beenden</button>`}<button type="button" class="btn personnel-small-btn" onclick="editPersonnelAbsence('${uId}','${id}')">✏️ Bearbeiten</button></div>`:due?'<span class="personnel-status-due">🔴 Rückkehr prüfen</span>':'<span class="personnel-status-active">● Läuft</span>';
        return `<div class="personnel-calendar-row ${due?'due':''} ${returned?'returned':''}"><div><b>${escapeHtml(personnelUserName(uId))}</b><span>${escapeHtml(getEmployeeRankLabels(uId).join(' · '))}</span></div><div><b>${escapeHtml(PERSONNEL_ABSENCE_TYPES[x.type]?.label||'Abwesenheit')}</b><span>${escapeHtml(formatPersonnelDate(x.startDate))} – ${escapeHtml(formatPersonnelDate(x.endDate))}</span></div><div class="personnel-calendar-note">${escapeHtml(x.note||'')}</div><div>${actions}</div></div>`;
    }).join(''):'<div class="personnel-empty-state">Keine Abwesenheiten für diesen Filter.</div>';
}
async function addPersonnelAbsence(){if(!getPersonnelPermissions().canManageAbsences)return;const uId=document.getElementById('personnelAbsenceEmployee')?.value||'',type=document.getElementById('personnelAbsenceType')?.value||'',startDate=document.getElementById('personnelAbsenceStart')?.value||'',endDate=document.getElementById('personnelAbsenceEnd')?.value||'',note=(document.getElementById('personnelAbsenceNote')?.value||'').trim();if(!uId||!PERSONNEL_ABSENCE_TYPES[type]||!startDate||!endDate){alert('Bitte Mitarbeiter, Art und Zeitraum vollständig angeben.');return;}if(endDate<startDate){alert('Das Enddatum darf nicht vor dem Startdatum liegen.');return;}if(getPersonnelActiveAbsence(uId)){alert('Für diesen Mitarbeiter gibt es bereits eine noch nicht abgeschlossene Abwesenheit.');return;}const now=Date.now(),author=personnelUserName(getUserAccountId(sessionUser)),authorId=getUserAccountId(sessionUser),ref=db.ref(`data/personnelAbsences/${uId}`).push(),updates={};updates[`data/personnelAbsences/${uId}/${ref.key}`]={type,startDate,endDate,note,status:'active',active:true,createdAt:now,createdBy:author,createdById:authorId};updates[`data/employeeAbsenceStatus/${uId}`]={type,active:true,updatedAt:now,updatedBy:author,updatedById:authorId};try{await db.ref().update(updates);await addPersonnelEvent(uId,'absence','Abwesenheit eingetragen',`${PERSONNEL_ABSENCE_TYPES[type].label} · ${formatPersonnelDate(startDate)} bis ${formatPersonnelDate(endDate)}`);logAdminAudit('Abwesenheit eingetragen',`${personnelUserName(uId)}: ${PERSONNEL_ABSENCE_TYPES[type].label} ${startDate}–${endDate}`);showToast('✅ Abwesenheit eingetragen.','success');}catch(err){console.error(err);showToast('⚠️ Abwesenheit konnte nicht gespeichert werden.','error',5000);}}
function editPersonnelAbsence(uId,id){if(!getPersonnelPermissions().canManageAbsences)return;const x=cachedPersonnelAbsences?.[uId]?.[id];if(!x||x.status==='returned'||x.active===false)return;activePersonnelAbsenceEditId=`${uId}:${id}`;renderPersonnelCalendar();}
function cancelPersonnelAbsenceEdit(){activePersonnelAbsenceEditId='';renderPersonnelCalendar();}
async function savePersonnelAbsenceEdit(uId,id){
    if(!getPersonnelPermissions().canManageAbsences)return;const current=cachedPersonnelAbsences?.[uId]?.[id];if(!current||current.status==='returned'||current.active===false)return;
    const type=document.getElementById(`personnelAbsenceEditType_${id}`)?.value||'',startDate=document.getElementById(`personnelAbsenceEditStart_${id}`)?.value||'',endDate=document.getElementById(`personnelAbsenceEditEnd_${id}`)?.value||'',note=(document.getElementById(`personnelAbsenceEditNote_${id}`)?.value||'').trim();
    if(!PERSONNEL_ABSENCE_TYPES[type]||!startDate||!endDate){alert('Bitte Art und Zeitraum vollständig angeben.');return;}if(endDate<startDate){alert('Das Enddatum darf nicht vor dem Startdatum liegen.');return;}
    const now=Date.now(),author=personnelUserName(getUserAccountId(sessionUser)),authorId=getUserAccountId(sessionUser),updates={};
    updates[`data/personnelAbsences/${uId}/${id}/type`]=type;updates[`data/personnelAbsences/${uId}/${id}/startDate`]=startDate;updates[`data/personnelAbsences/${uId}/${id}/endDate`]=endDate;updates[`data/personnelAbsences/${uId}/${id}/note`]=note;
    updates[`data/employeeAbsenceStatus/${uId}`]={type,active:true,updatedAt:now,updatedBy:author,updatedById:authorId};
    try{await db.ref().update(updates);activePersonnelAbsenceEditId='';renderPersonnelCalendar();await addPersonnelEvent(uId,'absence_edit','Abwesenheit bearbeitet',`${PERSONNEL_ABSENCE_TYPES[type].label} · ${formatPersonnelDate(startDate)} bis ${formatPersonnelDate(endDate)}`);logAdminAudit('Abwesenheit bearbeitet',`${personnelUserName(uId)}: ${PERSONNEL_ABSENCE_TYPES[type].label} ${startDate}–${endDate}`);showToast('✅ Abwesenheit wurde aktualisiert.','success');}
    catch(err){console.error(err);showToast('⚠️ Abwesenheit konnte nicht aktualisiert werden.','error',5000);}
}
async function endPersonnelAbsenceEarly(uId,id){
    if(!getPersonnelPermissions().canManageAbsences)return;const current=cachedPersonnelAbsences?.[uId]?.[id];if(!current||current.status==='returned'||current.active===false)return;
    if(!confirm(`Abwesenheit von ${personnelUserName(uId)} jetzt frühzeitig beenden? Der öffentliche Abwesenheitsstatus wird sofort entfernt.`))return;
    const now=Date.now(),author=personnelUserName(getUserAccountId(sessionUser)),authorId=getUserAccountId(sessionUser),updates={};updates[`data/personnelAbsences/${uId}/${id}/status`]='returned';updates[`data/personnelAbsences/${uId}/${id}/active`]=false;updates[`data/personnelAbsences/${uId}/${id}/returnedAt`]=now;updates[`data/personnelAbsences/${uId}/${id}/returnedBy`]=author;updates[`data/personnelAbsences/${uId}/${id}/returnedById`]=authorId;updates[`data/personnelAbsences/${uId}/${id}/closedReason`]='early_return';updates[`data/employeeAbsenceStatus/${uId}`]=null;
    try{await db.ref().update(updates);if(activePersonnelAbsenceEditId===`${uId}:${id}`)activePersonnelAbsenceEditId='';renderPersonnelCalendar();await addPersonnelEvent(uId,'return','Abwesenheit frühzeitig beendet',`${PERSONNEL_ABSENCE_TYPES[current.type]?.label||'Abwesenheit'} wurde vor dem geplanten Enddatum beendet`);logAdminAudit('Abwesenheit frühzeitig beendet',`${personnelUserName(uId)} wurde vorzeitig zurückgemeldet.`);showToast('✅ Abwesenheit wurde frühzeitig beendet.','success');}
    catch(err){console.error(err);showToast('⚠️ Abwesenheit konnte nicht beendet werden.','error',5000);}
}
async function confirmPersonnelReturn(uId,id){if(!getPersonnelPermissions().canManageAbsences)return;if(!confirm(`Rückkehr von ${personnelUserName(uId)} bestätigen? Der öffentliche Abwesenheitsstatus wird danach entfernt.`))return;const now=Date.now(),author=personnelUserName(getUserAccountId(sessionUser)),authorId=getUserAccountId(sessionUser),updates={};updates[`data/personnelAbsences/${uId}/${id}/status`]='returned';updates[`data/personnelAbsences/${uId}/${id}/active`]=false;updates[`data/personnelAbsences/${uId}/${id}/returnedAt`]=now;updates[`data/personnelAbsences/${uId}/${id}/returnedBy`]=author;updates[`data/personnelAbsences/${uId}/${id}/returnedById`]=authorId;updates[`data/employeeAbsenceStatus/${uId}`]=null;try{await db.ref().update(updates);await addPersonnelEvent(uId,'return','Rückkehr bestätigt','Abwesenheit beendet');logAdminAudit('Rückkehr bestätigt',`${personnelUserName(uId)} wurde aus der Abwesenheit zurückgemeldet.`);showToast('✅ Rückkehr bestätigt.','success');}catch(err){console.error(err);showToast('⚠️ Rückkehr konnte nicht bestätigt werden.','error',5000);}}

function renderPersonnelCreateForm(){const career=document.getElementById('personnelCreateCareer'),rank=document.getElementById('personnelCreateRank');if(!career||!rank)return;['personnelCreateInstructedBy','personnelCreateHiredBy'].forEach(id=>{const el=document.getElementById(id);if(!el)return;const old=el.value||'';el.innerHTML=personnelTrainerOptions(old);el.value=Array.from(el.options).some(o=>o.value===old)?old:'';});const c=career.value||'none',allowed=PERSONNEL_COMMON_RANKS.filter(k=>PERSONNEL_RANKS[k].track==='common-low');if(c==='doctor'||c==='both')allowed.push(...PERSONNEL_DOCTOR_RANKS);if(c==='paramedic'||c==='both')allowed.push(...PERSONNEL_PARAMEDIC_RANKS);allowed.push(...PERSONNEL_COMMON_RANKS.filter(k=>PERSONNEL_RANKS[k].track==='common-high'));const old=rank.value;rank.innerHTML=allowed.map(k=>`<option value="${k}" ${old===k?'selected':''}>${escapeHtml(PERSONNEL_RANKS[k].label)}</option>`).join('');if(!allowed.includes(old))rank.value='trainee';}
async function createPersonnelEmployee(){
    ...
    try{for(let i=0;i<25;i++,version++){try{ctx=await createSecondaryAuthAccount(uId,version,password,false);break;}catch(err){if(err?.code==='auth/email-already-in-use')continue;throw err;}}if(!ctx?.user)throw new Error('Kein freier technischer Zugang verfügbar.');secondaryDb=ctx.app.database();const todayIso=new Date().toLocaleDateString('sv-SE'),baseUser={accountId:uId,loginKey,vorname:v,nachname:n,dn,status:'pending',date:new Date().toLocaleDateString('de-DE'),einstellungsDatum:todayIso,roles:{mitarbeiter:true},photoUrl:'mdlogo.png',authUid:ctx.user.uid,authVersion:version};baseUser.serverPermissions=buildServerPermissions(baseUser);await secondaryDb.ref(`data/authIndex/${ctx.user.uid}`).set(uId);if(known.exists&&known.deleted)await secondaryDb.ref(`data/loginDirectory/${loginKey}`).update({version,accountId:uId,deleted:false});else await secondaryDb.ref(`data/loginDirectory/${loginKey}`).set({version,accountId:uId});await secondaryDb.ref(`data/users/${uId}`).set(baseUser);userStored=true;await closeSecondaryAuthAccount(ctx);ctx=null;const eff=getUserEffectivePermissions(sessionUser);if(eff.canManageMemberAccess||eff.isMasterAdmin)await db.ref(`data/users/${uId}/status`).set('approved');const author=personnelUserName(getUserAccountId(sessionUser)),authorId=getUserAccountId(sessionUser),now=Date.now();if(career!=='none'&&p.canManageCareer)await db.ref(`data/employeeCareerPaths/${uId}`).set({path:career,source:'manual',updatedAt:now,updatedBy:author,updatedById:authorId});if(p.canManageRanks)await db.ref(`data/employeeRanks/${uId}`).set({common:PERSONNEL_RANKS[rank]?.track?.startsWith('common')?rank:'',doctor:PERSONNEL_RANKS[rank]?.track==='doctor'?rank:'',paramedic:PERSONNEL_RANKS[rank]?.track==='paramedic'?rank:'',source:'manual',updatedAt:now,updatedBy:author,updatedById:authorId});if(p.canManageRecords){await db.ref(`data/personnelProfiles/${uId}`).set({phone:(document.getElementById('personnelCreatePhone')?.value||'').trim(),secondJob:(document.getElementById('personnelCreateSecondJob')?.value||'').trim(),additionalFunctions:(document.getElementById('personnelCreateFunctions')?.value||'').trim(),instructedBy:(document.getElementById('personnelCreateInstructedBy')?.value||'').trim(),hiredBy:(document.getElementById('personnelCreateHiredBy')?.value||'').trim(),swornIn:document.getElementById('personnelCreateSworn')?.value||'no',firstAidCourse:document.getElementById('personnelCreateFirstAid')?.value||'no',treatmentInstruction:document.getElementById('personnelCreateTreatment')?.value||'no',internalStatus:'none',updatedAt:now,updatedBy:author,updatedById:authorId});}await addPersonnelEvent(uId,'hire','Einstellung',`${formatPersonnelDate(todayIso)} · Start-Rang: ${PERSONNEL_RANKS[rank]?.label||rank}`);logAdminAudit('Mitarbeiter angelegt',`${v} ${n} (${formatStaffDn(dn)}) wurde über die Personalabteilung angelegt.`);['personnelCreateFirstName','personnelCreateLastName','personnelCreateDn','personnelCreatePassword','personnelCreatePhone','personnelCreateSecondJob','personnelCreateFunctions','personnelCreateInstructedBy','personnelCreateHiredBy'].forEach(id=>{const e=document.getElementById(id);if(e)e.value='';});['personnelCreateSworn','personnelCreateFirstAid','personnelCreateTreatment'].forEach(id=>{const e=document.getElementById(id);if(e)e.value='no';});showToast((eff.canManageMemberAccess||eff.isMasterAdmin)?'✅ Mitarbeiter wurde angelegt und freigeschaltet.':'✅ Mitarbeiter wurde angelegt und wartet auf Freischaltung.','success',5000);activePersonnelEmployeeId=uId;switchPersonnelView('files');}
...
function refreshPersonnelModule(){
    db.ref('data/employeeRanks').off();db.ref('data/employeeAbsenceStatus').off();db.ref('data/personnelRecords').off();db.ref('data/personnelAbsences').off();db.ref('data/personnelProfiles').off();db.ref('data/personnelDepartures').off();
