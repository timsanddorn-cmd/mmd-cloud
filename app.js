// ============================================================
//  MMD CLOUD – Medical Center Web-App  |  app.js  v6.8.10
...
async function importPersonnelMasterlistDefaults(){
    ...
    try{
        ...
        for(const seed of PERSONNEL_MASTERLIST_SEED){
            ...
            if(p.canManageCareer&&seed.career&&seed.career!=='none'&&!careers[uId]?.path)set(`data/employeeCareerPaths/${uId}`,{path:seed.career,updatedAt:now,updatedBy:author,updatedById:authorId});
            if(p.canManageRanks&&seed.rank){
                const cur=ranks[uId]||{};if(!cur.common&&!cur.doctor&&!cur.paramedic)set(`data/employeeRanks/${uId}`,{...buildMasterlistRankState(seed.rank),updatedAt:now,updatedBy:author,updatedById:authorId});
            }
        }
...
function renderPersonnelEmployeeDetail(){
    ...
    const p=getPersonnelPermissions(),career=getEmployeeCareerPathKey(uId),ranks=getEmployeeRankData(uId),record=cachedPersonnelRecords?.[uId]||{},profile=getPersonnelProfile(uId),internal=getPersonnelInternalStatus(uId),timeline=[],profileEditing=personnelProfileEditingId===uId;
    ...
    <div class="personnel-file-grid">
      ${p.canViewRecords?`<section class="personnel-card personnel-profile-card"><div class="personnel-card-title-row"><h4>🪪 Stammdaten & Qualifikationen</h4>${p.canManageRecords&&!profileEditing?`<button type="button" class="personnel-edit-icon" onclick="togglePersonnelProfileEdit('${uId}',true)" title="Stammdaten bearbeiten" aria-label="Stammdaten bearbeiten">✏️</button>`:''}</div><div class="personnel-readonly-row"><span>Eingestellt am</span><b>${escapeHtml(formatPersonnelDate(user.einstellungsDatum||getPersonnelMasterlistSeedForUser(user)?.employmentDate))}</b></div><div class="personnel-readonly-row"><span>Diensttage</span><b>${getServiceDaysForUser(user)??'—'}</b></div>
      <label>Telefonnummer</label><input id="personnelProfilePhone" value="${escapeHtml(profile.phone||'')}" ${profileEditing?'':'disabled'} placeholder="Optional"><label>Zweitjob</label><input id="personnelProfileSecondJob" value="${escapeHtml(profile.secondJob||'')}" ${profileEditing?'':'disabled'} placeholder="Optional"><label>Interne Funktionen / Zusatzaufgaben</label><textarea id="personnelProfileFunctions" rows="3" ${profileEditing?'':'disabled'} placeholder="z. B. Personalabteilung, Luftrettung …">${escapeHtml(profile.additionalFunctions||'')}</textarea>
      <div class="personnel-two-col"><div><label>Eingewiesen durch</label><input id="personnelProfileInstructedBy" value="${escapeHtml(profile.instructedBy||'')}" ${profileEditing?'':'disabled'}></div><div><label>Eingestellt durch</label><input id="personnelProfileHiredBy" value="${escapeHtml(profile.hiredBy||'')}" ${profileEditing?'':'disabled'}></div></div>
      <div class="personnel-three-col"><div><label>Vereidigung</label><select id="personnelProfileSworn" ${profileEditing?'':'disabled'}>${personnelQualificationOptions(profile.swornIn||'unknown')}</select></div><div><label>Erste-Hilfe-Kurs</label><select id="personnelProfileFirstAid" ${profileEditing?'':'disabled'}>${personnelQualificationOptions(profile.firstAidCourse||'unknown')}</select></div><div><label>Behandlungseinweisung</label><select id="personnelProfileTreatment" ${profileEditing?'':'disabled'}>${personnelQualificationOptions(profile.treatmentInstruction||'unknown')}</select></div></div>
      <label>Interner Personalstatus</label><select id="personnelProfileInternalStatus" ${profileEditing?'':'disabled'}>${Object.entries(PERSONNEL_INTERNAL_STATUSES).map(([k,v])=>`<option value="${k}" ${internal===k?'selected':''}>${v.icon} ${escapeHtml(v.label)}</option>`).join('')}</select>
      ${p.canManageRecords&&profileEditing?`<div class="personnel-edit-actions"><button type="button" class="btn personnel-primary-btn" onclick="savePersonnelProfile('${uId}')">💾 Speichern</button><button type="button" class="btn personnel-secondary-btn" onclick="togglePersonnelProfileEdit('${uId}',false)">Abbrechen</button></div>`:''}</section>`:''}
      ${(p.canManageCareer||p.canManageRanks)?`<section class="personnel-card"><h4>🎖️ Laufbahn & Ränge</h4>${p.canManageCareer?`<label>Laufbahn</label><div class="personnel-inline-row"><select id="personnelCareerSelect"><option value="none" ${career==='none'?'selected':''}>Noch nicht festgelegt</option><option value="doctor" ${career==='doctor'?'selected':''}>Doctor</option><option value="paramedic" ${career==='paramedic'?'selected':''}>Paramedic</option><option value="both" ${career==='both'?'selected':''}>Doctor & Paramedic</option></select><button type="button" class="btn personnel-small-btn" onclick="savePersonnelCareer('${uId}')">Speichern</button></div>`:''}${p.canManageRanks?`<label>Gemeinsamer Rang</label><select id="personnelRankCommon">${buildPersonnelRankOptions(PERSONNEL_COMMON_RANKS,ranks.common)}</select>${career==='doctor'||career==='both'?`<label>Doctor-Rang</label><select id="personnelRankDoctor">${buildPersonnelRankOptions(PERSONNEL_DOCTOR_RANKS,ranks.doctor)}</select>`:''}${career==='paramedic'||career==='both'?`<label>Paramedic-Rang</label><select id="personnelRankParamedic">${buildPersonnelRankOptions(PERSONNEL_PARAMEDIC_RANKS,ranks.paramedic)}</select>`:''}<label>Grund der Rangänderung</label><select id="personnelRankReason"><option value="">Bitte wählen</option><option>Ausbildung</option><option>Wiedereinstellung</option><option>Sonder-Rankup</option></select><label>Zusatz / Begründung</label><input type="text" id="personnelRankNote" maxlength="180" placeholder="Optionaler Zusatz"><button type="button" class="btn personnel-primary-btn" onclick="savePersonnelRanks('${uId}')">🎖️ Ränge speichern</button>`:''}</section>`:''}
