...
let cachedEmployeeRanks={},cachedEmployeeAbsenceStatus={},cachedPersonnelRecords={},cachedPersonnelAbsences={},cachedPersonnelProfiles={},cachedPersonnelDepartures={},activePersonnelEmployeeId='',activePersonnelView='overview',personnelProfileEditingId='',activePersonnelSanctionEditId='';
...
function personnelQualificationOptions(selected){return ['unknown','yes','no'].map(k=>{const label=k==='yes'?'Ja':k==='no'?'Nein':'Nicht festgelegt';return `<option value="${k}" ${selected===k?'selected':''}>${label}</option>`;}).join('');}
...
function selectPersonnelEmployee(uId){activePersonnelEmployeeId=uId;personnelProfileEditingId='';activePersonnelSanctionEditId='';renderPersonnelEmployeeList();renderPersonnelEmployeeDetail();}
...
function renderPersonnelEmployeeDetail(){
    ...
    <div class="personnel-file-grid">
      ...
      <div class="personnel-two-col"><div><label>Eingewiesen durch</label><input id="personnelProfileInstructedBy" value="${escapeHtml(profile.instructedBy||'')}" ${p.canManageRecords?'':'disabled'}></div><div><label>Eingestellt durch</label><input id="personnelProfileHiredBy" value="${escapeHtml(profile.hiredBy||'')}" ${p.canManageRecords?'':'disabled'}></div></div>
